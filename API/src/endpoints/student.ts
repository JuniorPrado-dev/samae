...
await connection('tbdados_anamnese').insert({
  id_aluno: idAluno,
  q1: dadosFamiliares.q1,
  q2: dadosFamiliares.q2,
  q3: dadosFamiliares.q3,
  q4: dadosFamiliares.q4,
  q5: dadosFamiliares.q5,
  q6: dadosFamiliares.q6,
  q7: dadosFamiliares.q7,
  q8: dadosFamiliares.q8,
  q9: dadosFamiliares.q9,
  q10: dadosFamiliares.q10,
  q11: dadosFamiliares.q11,
  q12: dadosFamiliares.q12,
  q13: dadosFamiliares.q13,
  q14: dadosFamiliares.q14,
});
...
export async function getAnamnese(req: Request, res: Response) {
  try {
    ...
    const cabecalho = await connection('tbaluno_anamnese').where('id_aluno', id_aluno).first();
    ...
    const dadosFamiliares = await connection('tbdados_anamnese').where('id_aluno', id_aluno).first();
    ...
    const disciplinacao = await connection('tbdisciplinação_anamnese').where('id_aluno', id_aluno).first();

    const saudeAnamnese = await connection('tbsaude_anamnese').where('id_aluno', id_aluno).first();
    

    return res.json({ cabecalho, aspectosMotores, aspectosPerceptivos, atitudesAnamnese, dadosFamiliares, historicoAnamnese,
    aspectosEmocionais, sociabilidadeAnamnese, sonoAnamnese, disciplinacao, saudeAnamnese });
  } catch (error) {
    console.error(error);
    return res.send('Erro interno do servidor');
  }
